...
export default function ServerStatus() {
  ...
  return (
    <div className="p-7 overflow-y-auto h-full max-w-lg">
      <h1 className="text-lg font-bold text-slate-100">Statut serveur</h1>
      <p className="text-xs text-slate-500 mt-0.5 mb-5">Actualisé automatiquement.</p>

      {status === undefined && <p className="text-xs text-slate-500">Chargement…</p>}

      {status === null && (
        <div className="rounded-xl bg-base-900 border border-white/[0.05] px-5 py-8 text-center">
          <span className="material-symbols-rounded !text-[28px] text-slate-600">dns</span>
          <p className="text-xs text-slate-500 mt-2">Statut indisponible pour le moment.</p>
        </div>
      )}

      {status && (
        <div className="rounded-xl bg-base-900 border border-white/[0.05] p-5">
          ...
          {status.online && (
            <div>
              <div className="flex items-baseline gap-1.5">
                <span className="text-2xl font-bold text-slate-50">{status.playerCount}</span>
                {status.maxPlayers != null && (
                  <span className="text-sm text-slate-500">/ {status.maxPlayers} joueurs</span>
                )}
              </div>
              {status.maxPlayers > 0 && (
