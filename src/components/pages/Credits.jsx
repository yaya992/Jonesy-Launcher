    ...
    <div className="p-7 overflow-y-auto h-full max-w-lg">
      ...
      <div className="grid grid-cols-2 gap-2.5">
        {credits?.map((c) => (
          <div
            key={c.id}
            className="rounded-xl bg-base-900 border border-tint/[0.05] p-4 flex items-center gap-3"
          >
            ...
            <div className="min-w-0">
              <p className="text-sm font-semibold text-ink-200 truncate">{c.name}</p>
              <p className="text-[11px] text-ink-500 truncate">{c.role}</p>
            </div>
          </div>
        ))}
      </div>
