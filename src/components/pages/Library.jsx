...
export default function Library({ maintenance, canDownload }) {
  ...
  return (
    <div className="p-7 overflow-y-auto h-full">
      <h1 className="text-lg font-bold text-slate-100">Library</h1>
      <p className="text-xs text-slate-500 mt-0.5 mb-5">
        Installe, vérifie ou répare tes saisons et contenus.
      </p>

      {!games && <p className="text-xs text-slate-500">Chargement du catalogue…</p>}
      {games?.length === 0 && (
        <p className="text-xs text-slate-500">Aucun contenu disponible pour le moment.</p>
      )}
